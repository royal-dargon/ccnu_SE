	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="论坛分类" prop="luntanfenlei">
					<el-select :disabled="ro.luntanfenlei" v-model="ruleForm.luntanfenlei" placeholder="请选择论坛分类" >
						<el-option
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../python0ebx7/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
